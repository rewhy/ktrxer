...
static pkt_bw	bwres[256];
static uint32_t bw_num = 0;
...
int pkts_tx(int num, int len, int delay)
{
	...
	tx_s = get_current_ts(); // Beginning timestamps
	...
	tx_e = get_current_ts(); // Finishing timestamps
...
ip_process(char *data, struct timeval tv_rx, int len)
{
	...
	switch(iph->protocol) {
		case IPPROTO_UDP:
			res = bwres + (int)(len / 10);
			udph = (struct udphdr *)((char *) iph + (iph->ihl << 2));
			if(ntohs(udph->dest) > 10000 && ntohs(udph->dest) < 10010) {
				if(res->pkts == 0) {
					bw_num = bw_num > len / 10 ? bw_num : len / 10;
					res->tv_start = now;
...
void rx_test()
{
	...
	void print_rx_res() {
		double b = 0.0, e=0.0, d = 0.0;
		int i;
		for(i = 0; i <= bw_num; i++){
			b = (double)bwres[i].tv_start.tv_sec + (double)bwres[i].tv_start.tv_usec/1000000.0;
			e = (double)bwres[i].tv_end.tv_sec + (double)bwres[i].tv_end.tv_usec/1000000.0;
			d = e - b;
			if(d == 0)
				continue;
			printf("%d, %d, %d, %f\n",
					...
					bwres[i].pkts,
					bwres[i].bytes,
					(double)bwres[i].bytes * 8.0 / (1024.0 * 1024.0 * d));
		}
	}
